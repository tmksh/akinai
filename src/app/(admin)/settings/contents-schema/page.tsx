...
const fieldTypeConfig: Record<CustomFieldType, { label: string; icon: React.ElementType; color: string; category: 'basic' | 'media' | 'advanced' }> = {
  ...
  image_url_list: { label: '複数画像', icon: ImageIcon,   color: 'text-sky-400',     category: 'media' },
...
function FieldEditor({ fields, onChange, productSchema = [] }: FieldEditorProps) {
  ...
  const addField = () => {
    ...
    if (fields.some(f => f.key === newKey.trim())) {
      ...
      {isAdding && (
        <div className="rounded-xl border-2 border-dashed border-sky-200 dark:border-sky-900/50 bg-sky-50/50 dark:bg-sky-950/10 p-5 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button size="sm" onClick={addField} disabled={!newLabel.trim() || !newKey.trim()} className="btn-premium">
      ...
      {fields.length > 0 ? (
          ...
          {fields.map((field) => {
            ...
            return (
              <div key={field.id} className="group flex items-center gap-3 rounded-lg border bg-background px-4 py-3 hover:shadow-sm transition-shadow">
                ...
                <Badge variant="outline" className="text-[10px] px-1.5 h-5">{config.label}</Badge>
                {field.options && (
        ...
        !isAdding && (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">このコンテンツタイプ専用のカスタムフィールドを定義できます</p>
          </button>
        )
      )}
    </div>
...
export default function ContentSchemaSettingsPage() {
  ...
  useEffect(() => {
    if (organization?.contentFieldSchema) {
      setSchemaByType(organization.contentFieldSchema);
    }
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const { error } = await updateContentFieldSchema(organization.id, schemaByType);
      if (error) {
        toast.error('保存に失敗しました: ' + error);
      } else {
        toast.success('カスタムフィールドを保存しました');
        await refetch();
      }
    });
  };

  const updateTypeSchema = (type: string, fields: ContentFieldSchemaItem[]) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">コンテンツタイプごとにカスタムフィールドを定義します</p>
        </div>
        ...
        <CardContent className="pt-4">
          <div className="flex gap-3">
            ...
            <div className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
              <p className="font-medium">タイプごとに専用フィールドを設定できます</p>
              <p className="text-blue-600/80 dark:text-blue-400/80">
                ギャラリーにだけ「撮影場所」、ニュースにだけ「著者」などタイプ別に独立して管理されます。
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-sky-500" />フィールド定義
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="flex-wrap h-auto gap-1 mb-6">
              {contentTypes.map(([type, config]) => {
                const Icon = config.icon;
                const count = schemaByType[type]?.length ?? 0;
                return (
