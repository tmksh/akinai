...
export default function QuotesClient({
  ...
  const [quotes, setQuotes] = useState<QuoteWithItems[]>(initialQuotes ?? []);
  const [isLoading, setIsLoading] = useState(!initialQuotes);
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');

  // データ取得
  useEffect(() => {
    if (!organizationId || initialQuotes) return;
    setIsLoading(true);
    getQuotes(organizationId).then(({ data, error }) => {
      if (error) console.error('Failed to fetch quotes:', error);
      setQuotes(data || []);
      setIsLoading(false);
    });
  }, [organizationId, initialQuotes]);
