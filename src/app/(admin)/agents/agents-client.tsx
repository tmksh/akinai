...
export default function AgentsClient({
  ...
  const [stats, setStats] = useState<AgentStats>(initialStats ?? {
    total: 0, active: 0, inactive: 0, pending: 0, totalSales: 0, totalCommission: 0,
  });
  ...
  useEffect(() => {
    if (!initialAgents) fetchData();
  }, [fetchData, initialAgents]);
  ...
  const handleNewAgent = useCallback(async () => {
    ...
    // 新しい代理店コードを生成
    const code = await generateAgentCode(organizationId);
  ...
  const handleFormSubmit = useCallback(async (data: AgentFormData) => {
    ...
    fetchData();
  }, [editingAgent, organizationId, fetchData]);
