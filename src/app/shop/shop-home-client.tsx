...
import type { ShopProduct, ShopCategory, ShopContent } from '@/lib/actions/shop';

export interface ShopHomeInitialData {
...
// ----- 注目製品セクション -----
function FeaturedProductsSection({ products }: { products: ShopProduct[] }) {
  return (
    <section
...
// ----- カテゴリーセクション -----
function CategorySection({ categories }: { categories: ShopCategory[] }) {
...
// ----- 人気製品セクション -----
function PopularProductsSection({ products }: { products: ShopProduct[] }) {
  return (
...
// ----- 記事・特集セクション -----
function ArticlesSection({ articles }: { articles: ShopContent[] }) {
...
// ----- ニュースセクション -----
function NewsSection({ newsItems }: { newsItems: ShopContent[] }) {
...
function renderSection(id: ShopSectionId, data: ShopHomeInitialData) {
  switch (id) {
    case 'hero': return <HeroSection />;
    case 'concept': return <ConceptSection />;
    case 'featured': return <FeaturedProductsSection products={data.featuredProducts} />;
    case 'category': return <CategorySection categories={data.categories} />;
    case 'articles': return <ArticlesSection articles={data.contents.slice(0, 4)} />;
    case 'popular': return <PopularProductsSection products={data.popularProducts} />;
    case 'business': return <BusinessSection />;
    case 'news': return <NewsSection newsItems={data.contents.slice(0, 5)} />;
    case 'instagram': return <InstagramSection />;
    case 'brand': return <BrandImageSection />;
    default: return null;
  }
}

export default function ShopHomeClient({ initialData }: { initialData: ShopHomeInitialData }) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--shop-color-background, #ffffff)' }}>
      {enabledSections.map((section) => (
        <div key={section.id}>{renderSection(section.id, initialData)}</div>
      ))}
    </div>
  );
}
